...
var BB10Sender = new bb10.PushInitiator(SETTINGS.get('BB10_APPID'), SETTINGS.get('BB10_PASSWORD'),  SETTINGS.get('BB10_CPID'));
...
var sendPush = function(pushId, data, callback) {   
	...
	if (regIdsBB10[0] != undefined){	
		messageBB10 = new bb10.PushMessage(SETTINGS.get('stratton.bb10.push.'+SETTINGS.get('BB10_APPNAME')+'@'+date()), data.message);
		messageBB10.addAllRecipients(regIdsBB10);
		messageBB10.setDeliveryMethod(SETTINGS.get('BB10_DELIVERYMETHOD'));

		parallel.add(function(done){
			BB10Sender.push(message, function(err, result) {
				if(err) 
					done(0, ' BB10: ' +  err.message + '  ' + err.statusCode);
				else 
					done(0,1);
			});		
		})
