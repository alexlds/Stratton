...
var sendPush = function(pushId, title, key1, value1, key2, value2, key3, value3, key4, value4, key5, value5) {
	...
	console.log('arguments.pushId: ' + arguments[0]);
	...
	if (pushId.length>64){
		...
		var numKeyValue = (arguments.length - 2)/2;
		if (numKeyValue%2==0 && numKeyValue<6)
		{
			for (var i = 0; i<numKeyValue; i++)
			{
				message.addDataWithKeyValue(arguments[i+2], arguments[i+2]);
			}
			var registrationIds = [];
			registrationIds.push(pushId);

			var pushResult = GCMSender.send(message, registrationIds, 4, function (err, result) {
				if (result.success === 1)
					return ('OK');
				else
			...
			return (new restify.InvalidArgumentError('Invalid number of arguments');
	}
